...
# DISABLED: Calendar must use app.emailpilot.ai shared client library only
@router.get("/clients")
async def get_all_clients_redirect(request: Request) -> Dict[str, Any]:
    """Redirects to app.emailpilot.ai - this service does not maintain a client list"""
    return {
        "status": "error",
        "message": "This endpoint is disabled. Please use https://app.emailpilot.ai/api/admin/clients for the shared client library",
        "redirect_url": "https://app.emailpilot.ai/api/admin/clients",
        "total_clients": 0,
        "clients": []
    }
