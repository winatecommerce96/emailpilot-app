...
class ClientKeyResolver:
    ...
    async def get_client_klaviyo_key(self, client_id: str) -> Optional[str]:
        ...
        try:
            ...
            existing_secret_name = client_data.get('klaviyo_secret_name')
...
# Singleton instance
_client_key_resolver = None

def get_client_key_resolver(
    db: firestore.Client = Depends(get_db),
    secret_manager: SecretManagerService = Depends(get_secret_manager_service),
) -> ClientKeyResolver:
    """Get singleton instance of ClientKeyResolver"""
    global _client_key_resolver
    if _client_key_resolver is None:
        _client_key_resolver = ClientKeyResolver(db=db, secret_manager=secret_manager)
    return _client_key_resolver