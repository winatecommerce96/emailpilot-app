...
async def generate_calendar_plan(
    ...
    try:
        ...
        # Check Enhanced MCP Gateway connection if using real data
        mcp_available = False
        if use_mcp_data:
            try:
                # Use the Enhanced MCP Gateway for better capabilities
                import httpx
                async with httpx.AsyncClient() as client:
                    # Validate client setup via gateway
                    validation_response = await client.get(
                        f"http://localhost:8000/api/mcp/gateway/clients/{client_id}/validate"
                    )
                    
                    if validation_response.status_code == 200:
                        validation = validation_response.json()
                        mcp_available = validation.get("valid", False)
                        
                        if mcp_available:
                            logger.info(f"✅ Enhanced MCP Gateway available for {client_id}")
                            logger.info(f"   Client: {validation.get('client_name')}")
                            logger.info(f"   Has Secret: {validation.get('has_secret_reference')}")
                        else:
                            logger.warning(f"⚠️ MCP Gateway validation failed for {client_id}: {validation.get('error')}")
                    else:
                        logger.warning(f"MCP Gateway not responding for {client_id}, will use historical data")
                        
            except Exception as e:
                logger.error(f"MCP Gateway connection check failed: {e}")
                mcp_available = False
