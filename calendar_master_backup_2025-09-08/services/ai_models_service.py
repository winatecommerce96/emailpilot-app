...
class AIModelsService:
    ...
    async def execute_direct(self, prompt: str, provider: str, model: Optional[str] = None) -> str:
        """Execute a prompt directly without template lookup"""
        if provider not in self._api_clients:
            raise ValueError(f"Provider {provider} not configured or API key missing")
        
        if provider == "openai":
            return await self._execute_openai(prompt, model or "gpt-4o")
        elif provider == "gemini":
            return await self._execute_gemini(prompt, model or "gemini-1.5-pro")
        elif provider == "anthropic" or provider == "claude":
            return await self._execute_claude(prompt, model or "claude-3-5-sonnet-20241022")
        else:
