// MCP Testing Interface Component
// Provides visual feedback for production testing of MCP Management System
...
const MCPTestingInterface = () => {
    ...
    const testPhases = [
        {
            id: 'deployment',
            name: 'Phase 1: Verify Deployment',
            tests: [
                { id: 'check_status', name: 'Check deployment status', endpoint: '/api/health' },
                { id: 'verify_frontend', name: 'Verify frontend components', endpoint: null },
                { id: 'verify_api', name: 'Verify API endpoints', endpoint: '/api/mcp/models' }
            ]
        },
        {
            id: 'functional',
            ...
            tests: [
                { id: 'create_client', name: 'Create test client', endpoint: '/api/mcp/clients' },
                { id: 'test_connections', name: 'Test provider connections', endpoint: '/api/mcp/clients/{id}/test' },
                { id: 'execute_tools', name: 'Execute MCP tools', endpoint: '/api/mcp/execute' }
            ]
        },
        {
            id: 'integration',
            ...
            tests: [
                ...
                { id: 'multi_model', name: 'Test multi-model comparison', endpoint: '/api/mcp/execute' }
            ]
        },
        {
            id: 'security',
            ...
            tests: [
                { id: 'rate_limiting', name: 'Test rate limiting', endpoint: '/api/mcp/execute' },
                { id: 'api_security', name: 'Verify API key security', endpoint: null },
                { id: 'performance', name: 'Measure response times', endpoint: '/api/mcp/execute' }
            ]
        },
        {
            id: 'monitoring',
            ...
            tests: [
                { id: 'check_logs', name: 'Check application logs', endpoint: null },
    ...
    const runTest = async (phase, test) => {
        ...
        try {
            ...
            // Replace {id} with selected client ID if needed
            let endpoint = test.endpoint;
            if (endpoint.includes('{id}') && selectedClient) {
                endpoint = endpoint.replace('{id}', selectedClient.id);
            }

            const response = await fetch(endpoint, {
                method: test.id === 'create_client' ? 'POST' : 'GET',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
                    'Content-Type': 'application/json'
                },
                body: test.id === 'create_client' ? JSON.stringify({
            ...
            if (response.ok) {
                const data = await response.json();
                updateTestStatus(phase.id, test.id, 'success', `✅ ${test.name} passed`);
                
                // Store created client for subsequent tests
                if (test.id === 'create_client' && data.id) {
                    setSelectedClient(data);
                }
                return true;
            } else {
                const error = await response.text();
                updateTestStatus(phase.id, test.id, 'error', `Failed: ${error}`);
                return false;
            }
        } catch (error) {
            updateTestStatus(phase.id, test.id, 'error', `Error: ${error.message}`);
            return false;
        }
    };
    ...
    const exportTestResults = () => {
        ...
        a.href = url;
        a.download = `mcp-test-report-${Date.now()}.json`;
        a.click();
    };
    ...
    const generateCurlCommands = () => {
        ...
        commands.push('# Get auth token');
        commands.push(`TOKEN="${token}"`);
        commands.push('');
        commands.push('# Test MCP models endpoint');
        commands.push(`curl -X GET https://emailpilot.ai/api/mcp/models \\`);
        commands.push(`  -H "Authorization: Bearer $TOKEN"`);
        commands.push('');
        
        if (selectedClient) {
            commands.push('# Test specific client');
            commands.push(`curl -X GET https://emailpilot.ai/api/mcp/clients/${selectedClient.id} \\`);
            commands.push(`  -H "Authorization: Bearer $TOKEN"`);
            commands.push('');
            commands.push('# Test connection');
            commands.push(`curl -X POST https://emailpilot.ai/api/mcp/clients/${selectedClient.id}/test \\`);
            commands.push(`  -H "Authorization: Bearer $TOKEN" \\`);
        ...
        <div className="p-6 max-w-7xl mx-auto">
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
                <div className="flex justify-between items-center mb-4">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900">MCP Production Testing Suite</h2>
                        <p className="text-gray-600 mt-1">Automated testing interface for MCP Management System</p>
                    </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-semibold">Manual Testing Commands</h3>
                    <button
                        onClick={() => {
